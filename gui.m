...
% Last Modified by GUIDE v2.5 26-May-2022 15:43:47
...
[nama_file1, nama_path1]=uigetfile(...
    {'*.bmp; *.png; *.jpg', 'Inputkan citra (*.bmp, *.jpg, *.png)';
    '*.bmp' , 'File Bitmap(*.bmp)';
    '*.png' , 'File Png(*.png)';
    '*.jpg' , 'File Jpeg (*.jpg)';
    '*.*', 'Semua File (*.*)'},...
...
if ~isequal (nama_file1, 0);
    ...
    %output citra awal yg dipilih pada axes
else
    return
end
...
%proses ekstraksi R,G,B citra input
R = rgb(:,:,1);
G = rgb(:,:,2);
B = rgb(:,:,3);
 
%proses Invert rata" R,G,B
mR = 1/(mean(mean(R)));
mG = 1/(mean(mean(G)));
mB = 1/(mean(mean(B)));
 
%mencari nilai invert maksimal
...
%Objek yang terlihat akan dimarking
for i=1:numind
    bin(r(i),c(i)) = 1;
end
...
%rata" hasil dari ekstraksi citra
